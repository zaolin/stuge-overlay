...
IUSE="alsa dbus debug ntp video_cards_intel video_cards_radeon"
...
src_install() {
	...
	use dbus || {
		# delete the dbus_send function
		find "${D}" -type f -name HOWTO.hooks -exec sed -i '/^14: dbus_send$/,$d' '{}' +
		find "${D}" -type f -name functions -exec sed -i '/^# Like regular dbus-send/,$d' '{}' +
	}
